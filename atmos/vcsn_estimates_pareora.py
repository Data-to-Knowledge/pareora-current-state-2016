...
sites = ['403711', '405711', '404810', '405910', '414110']
...
### Get station data
ts_summ1 = mssql.rd_sql(server, database, ts_summ_table, where_col={'ExtSiteID': sites,'DatasetTypeID': [15]})
...
ts1 = mssql.rd_sql(server, database, ts_table, ['DateTime', 'Value'], where_col={'ExtSiteID': ['414110'],'DatasetTypeID': [15]})
ts1['DateTime'] = pd.to_datetime(ts1['DateTime'])
ts1.set_index('DateTime', inplace=True)
year_sum2 = ts1.resample('A-JUN').sum()['1972':'2015']

year_sum2.describe()

ts1 = mssql.rd_sql(server, database, ts_table, ['DateTime', 'Value'], where_col={'ExtSiteID': ['403711'],'DatasetTypeID': [15]})
ts1['DateTime'] = pd.to_datetime(ts1['DateTime'])
ts1.set_index('DateTime', inplace=True)
year_sum2 = ts1.resample('A-JUN').sum()['1972':'2015']
...
ts1 = mssql.rd_sql(server, database, ts_table, ['DateTime', 'Value'], where_col={'ExtSiteID': ['404810'],'DatasetTypeID': [15]})
ts1['DateTime'] = pd.to_datetime(ts1['DateTime'])
ts1.set_index('DateTime', inplace=True)
year_sum2 = ts1.resample('A-JUN').sum()['1972':'2015']
